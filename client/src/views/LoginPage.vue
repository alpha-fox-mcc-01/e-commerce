 ...
 <div class="container max-w-md mx-auto xl:max-w-3xl h-full flex bg-white rounded-lg shadow overflow-hidden">
  ...
  <div class="w-full xl:w-1/2 p-8">
    <form method="post" action="#" @submit.prevent="userLogin">
      ...
      <span @click="changePage('signupform')" class="text-gray-700 text-sm font-semibold">Sign up
      </span>
      <div class="mb-4 mt-6">
      ...
      <div class="flex w-full mt-8">
        <button class="w-full bg-gray-800 hover:bg-grey-900 text-white text-sm py-2 px-4 font-semibold rounded focus:outline-none focus:shadow-outline h-10" type="button" @click="userLogin">Sign in
        </button>
      </div>
    </form>
  </div>
